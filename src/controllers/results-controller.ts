...
import { getBooksByGenreAndQuery, getEntityById } from "../db/queries";
...
export async function getResults(req: Request, res: Response) {
  ...
  if (genreId) {
    const genre = await getEntityById("genres")(genreId);
  ...
  await renderView<QueryRenderOptions>(res, {
    ...
    currentPage: page,
    selectedGenre,
    baseQueryString: buildBaseQueryString(req, ["currentPage"]),
    noGenreQueryString: buildBaseQueryString(req, ["genreId", "currentPage"]),
  });
}
